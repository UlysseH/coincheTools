...
object Main_01 extends IOApp {
  def run(args: List[String]): IO[ExitCode] = {
    for {
      ...
      tricks = GameV2.computeTricksFromGameState(res2.get).get
      ...
      randomStartBestPoints = List.fill(100)(
        GameV2.randomGamesAllSuitsRandomHandMapIsolateBest(player1, 1000)
      )

      bySuit = randomStartBestPoints
        .groupBy(_._2)
        .map((suit, l) => (suit, l.map(_._1).sum / l.length))

      _ <- IO.println(bySuit)

      _ <- IO.println(
        randomStartBestPoints.map(_._1).sum / randomStartBestPoints.length
      )
